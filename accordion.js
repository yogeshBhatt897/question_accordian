...
accordionButtons.forEach((accordion) => {
  // For each accordion button, add the click event listener
  accordion.onclick = function () {
    // Toggle the 'is-open' class to expand/collapse the content
    this.classList.toggle("is-open");

    // Get the next sibling element (the content of the accordion)
    let content = this.nextElementSibling;

    // If the content is expanded (i.e., it has the 'is-open' class), collapse it
  ...
  // Add keyboard event listeners to handle Enter/Space for accessibility
  accordion.addEventListener('keydown', (event) => {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault(); // Prevent default space bar scrolling
      accordion.click(); // Trigger the click event to toggle the accordion
    }
  });
});
